...
<script>
     // Wish List Javascript code Start
   //  $(document).ready(function () {
    function toggle(id) {
        var heartButton = document.getElementById("wishlistButton"+id);
       ///console.log(heartButton);
        
        if (heartButton.classList.contains("red")) {
            heartButton.classList.remove("red");
            console.log('check envent remove call ');
            addremovewishlist('remove', id);
        } else {
                heartButton.classList.add("red");
                addremovewishlist('add', id);
                console.log('check envent call ');
        }
    }
    ...
    function addremovewishlist(action, product_id) {
        var user_id = "<?= $user->auth()->id; ?>";
        $.ajax({
            method:'POST',
            mode: "cors",
            url: "<?= DOMAIN ?>/wish-list.php",
            data: { action: action, 
                        ...
                        <div class="panel-body" style="padding: 15px;">
                            ...
                            <?php foreach ($query as $row) {
                             ...
                             if($user->auth()){
                                ...
                                $product_id = $row->id;
                                $wishListData = $wishList->getWishList($user_id, $product_id);
                                    ...
                                    <div class=" mb-0" style="padding: 0 !important;border-radius:0px;">
                                        ...
                                        <div class="mb-10">
                                            ...
                                            <a href="<?= DOMAIN ?>/<?= $slug ?>/<?= $row->seo_url ?>">
                                                <img src="<?= DOMAIN ?>/product-images/<?= $image->id ?>.<?= $image->ext ?>" data-first="<?= DOMAIN ?>/product-images/<?= $image->id ?>.<?= $image->ext ?>" data-second="<?= DOMAIN ?>/product-images/<?= $image->id ?>.<?= $image->ext ?>" alt="<?= $image->alt ?>" alt="<?= $image->alt ?>" style="width: 100%;border-radius: 10px;">
                                            </a>
                                        </div>
                                        ...
                                        <a class="prod-list-link" href="<?= DOMAIN ?>/<?= $slug ?>/<?= $row->seo_url ?>">
                                            ...
                                            <div class="">
                                                ...
                                                <div class="<?= $col ?>" style="padding: 0; width: 35%;">
                                        ...
                                        <?php if(isset($user->auth()->id)){?>
                                            <div class="col-xs-6" style="width: 15%; padding-left: 0px">
                                                <button id="wishlistButton<?= $row->id ?>" class="heart" onclick="toggle(<?= $row->id ?>)">❤</button>
                                            </div> 
                                        <?php } ?>
